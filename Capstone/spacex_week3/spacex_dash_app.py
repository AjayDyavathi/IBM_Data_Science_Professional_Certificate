...
launch_sites = [{'label': 'ALL Sites', 'value': 'ALL Sites'}] + [{'label': site, 'value': site} for site in spacex_df["Launch Site"].unique()]
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                        style={'textAlign': 'center', 'color': '#503D36',
                                               'font-size': 40}),
                                # TASK 1: Add a dropdown list to enable Launch Site selection
                                # The default select value is for ALL sites
                                dcc.Dropdown(id='site-dropdown', options = launch_sites, value = 'ALL Sites', placeholder = "Choose Launch Site", searchable = True),
                                html.Br(),
...
def scatter(x_, y_):
    if x_ == 'ALL Sites':
        new_df = spacex_df
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= y_[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= y_[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    else:
        new_df = spacex_df[spacex_df["Launch Site"] == x_]
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= y_[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= y_[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    return fig2
